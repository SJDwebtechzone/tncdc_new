...
import { fetchAffiliations, addAffiliationToServer, updateAffiliationToServer, deleteAffiliationFromServer } from '@/store/websiteSlice';
...
export default function WebsiteAffiliationsPage() {
    ...
    const handleOpenModal = (affiliation = null) => {
        if (affiliation) {
            ...
            setFormData({
                ...
                image: null,
                imagePreview: affiliation.image || ''
            });
        } else {
            setEditingId(null);
    ...
    const handleSave = async (e) => {
        ...
        try {
            if (editingId) {
                await dispatch(updateAffiliationToServer({ id: editingId, formData: submitData })).unwrap();
                toast.success("Affiliation updated successfully");
            } else {
                await dispatch(addAffiliationToServer(submitData)).unwrap();
                toast.success("Affiliation added successfully");
            }
            setIsModalOpen(false);
            setFormData({ title: '', subtitle: '', image: null, imagePreview: '' });
        } catch (error) {
            toast.error(error || "Failed to save affiliation");
        } finally {
    ...
    const handleDelete = async (id) => {
        if (window.confirm("Remove this affiliation?")) {
            try {
                await dispatch(deleteAffiliationFromServer(id)).unwrap();
                toast.success("Affiliation removed");
            } catch (error) {
                toast.error(error || "Failed to remove affiliation");
            }
        }
    };
                                ...
                                <div className="flex flex-col gap-4">
                                    ...
                                    {formData.imagePreview && (
                                        <div className="relative w-full aspect-[4/3] bg-gray-50 rounded-sm border border-gray-100 overflow-hidden group">
                                            ...
                                            <button
                                                type="button"
                                                onClick={() => setFormData(p => ({ ...p, image: null, imagePreview: editingId ? (affiliations.find(a => a.id === editingId)?.image || '') : '' }))}
