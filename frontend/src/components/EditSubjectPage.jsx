...
import { fetchLanguages, fetchSubjects } from '@/store/courseSlice';
...
const EditSubjectPage = () => {
    ...
    const [questionBanks, setQuestionBanks] = useState([
        { id: Date.now(), language: '', examType: 'Both', file: null }
    ]);
    ...
    useEffect(() => {
        ...
        // Load current subject from Redux store or fetch from API
        const existingSubject = subjects.find(s => s.id === parseInt(id));
        if (existingSubject) {
    ...
    const handleRemoveQuestionBank = (qbId) => {
        if (questionBanks.length > 1) {
            setQuestionBanks(questionBanks.filter(qb => qb.id !== qbId));
        }
    };

    const handleFileChange = (qbId, file) => {
        setQuestionBanks(questionBanks.map(qb =>
            qb.id === qbId ? { ...qb, file } : qb
        ));
    };

    const handleExamTypeChange = (qbId, type) => {
        setQuestionBanks(questionBanks.map(qb =>
            qb.id === qbId ? { ...qb, examType: type } : qb
        ));
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await axios.put(`${BASE_URL}/api/subjects/${id}`, { name: subjectName });

            // 2. If any new question banks with files, submit via recount
            const banksWithFiles = questionBanks.filter(qb => qb.file);
            if (banksWithFiles.length > 0) {
                const formData = new FormData();
                formData.append('questionBanks', JSON.stringify(banksWithFiles.map(qb => ({
                    language: qb.language,
                    examType: qb.examType
                }))));
                banksWithFiles.forEach(qb => {
                    formData.append('files', qb.file);
                });
                await axios.patch(`${BASE_URL}/api/subjects/${id}/recount`, formData);
            }
    ...
    return (
        <div className="w-full space-y-6 pb-10">
            ...
            <div className="flex items-center gap-4">
                ...
                <h1 className="text-2xl font-bold text-[#1e293b]">Edit Subject</h1>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <form onSubmit={handleSubmit} className="p-8 space-y-8">
                    ...
                    <div className="space-y-6">
                        <h3 className="text-lg font-bold text-[#1e293b] border-b pb-2">
                            Add New Question Bank Files
                        </h3>
                        <p className="text-sm text-gray-500 -mt-4">Upload new question bank files to add to this subject. Existing files will remain.</p>

                        {questionBanks.map((qb) => (
                            <div key={qb.id} className="p-6 rounded-xl border border-gray-100 bg-gray-50/30 space-y-6">
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <select
                                            ...
                                            value={qb.language}
                                            onChange={(e) => {
                                                setQuestionBanks(questionBanks.map(b =>
                                                    b.id === qb.id ? { ...b, language: e.target.value } : b
                                                ));
                                            }}
                                        ...
                                        <div className="flex gap-2">
                                            {['Final Exam', 'Mock Test', 'Both'].map((type) => (
                                                <button
                                                    ...
                                                    className={`flex-1 h-12 px-4 rounded-lg text-sm font-medium border transition-all flex items-center justify-center gap-2 ${qb.examType === type
